...
### 5. 예외사항 보고
- 구현 중 임시 처리, 우회 구현, 미완료 항목, 데이터/로컬라이제이션 미적용, 테스트 미실행, 에디터 후속 작업 필요사항이 발생하면 반드시 사용자에게 보고합니다.
- 보고 시에는 누락 없이 **리스트 형태**로 정리합니다.
- 사용자 경험에 영향을 주는 항목은 구현 완료 보고 전에 반드시 먼저 알립니다.
...
### 성능 고려사항
- **GetComponent 사용 최소화**: `GetComponent<T>()`는 비용이 높은 연산입니다.
  - **원칙**: 컴포넌트 참조는 외부에서 주입받거나, SerializeField로 할당합니다.
  - **필요시**: Awake()나 Initialize()에서 **딱 한 번만** 호출하여 캐싱합니다.
  - **금지**: Update()나 반복 호출되는 메서드 내에서 사용 금지
...
```csharp
// ❌ 나쁜 예시: GetComponent 반복 호출
public class BadExample : MonoBehaviour
{
    private void Update()
    {
        // 매 프레임 GetComponent 호출 - 매우 느림!
        var rigidbody = GetComponent<Rigidbody>();
        rigidbody.AddForce(Vector3.up);
    }
}

// ✅ 좋은 예시 1: SerializeField로 할당 (최선)
public class GoodExample1 : MonoBehaviour
{
    [SerializeField] private Rigidbody cachedRigidbody; // Inspector에서 할당
...
// ✅ 좋은 예시 3: 외부 주입 (의존성 주입 - 최고)
public class GoodExample3 : MonoBehaviour
{
    private Rigidbody cachedRigidbody;
...
- 인코딩 변환으로 한글이 깨졌다면 구현을 진행하지 말고 즉시 원본을 복구한 뒤 다시 작업합니다.
